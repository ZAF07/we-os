...
2. **Enable Organizations**, with *"create an organization on sign-up"* on. One Organization
...
- **AC 7's run-launch control.** The workspace reads a deliverable back but has no
  start-a-run button; run-launching UI belongs to slice 11. *Overturn this if you disagree
  — it is a narrowing of the original criterion, not something the criterion granted.*
...
1. **The verifier read the wrong claim.** It looked for `org_id` — Clerk session token **v1**,
   ...
   object (`{id, slg, rol, per, fpm}`), so *every real Clerk token would have been rejected*
