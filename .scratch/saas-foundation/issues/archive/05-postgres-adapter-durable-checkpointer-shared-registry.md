...
- [x] More than one worker can run without breaking the one-active-run-per-campaign guard. The claim is a partial unique index on `(tenant_id, slug) WHERE status = 'running'`, taken with `INSERT … ON CONFLICT DO NOTHING` rather than check-then-insert. **Partial:** run _traces_ are still node-local files, so with several workers a run's status is answerable anywhere but its SSE stream is not — recorded in ADR-0024.
...
- Commit: 6a37e9dea3683961ce93e142a22e9b6c98deebb8
