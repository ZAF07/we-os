...
- [ ] `TavilyWebSearch.search` returns the top-10-by-`score` results (from an
...
- [ ] `MARKETING_OS_WEB_BACKENDS` default is `tavily,google,duckduckgo`;
      `MARKETING_OS_TAVILY_SEARCH_DEPTH` defaults to `basic` and drives depth.
- [ ] Missing `MARKETING_OS_TAVILY_API_KEY` → Tavily omitted from the chain +
      warning logged; run proceeds on Playwright.
- [ ] 429/432/433/5xx/network/timeout raise recoverable `ToolError` → chain
      falls through to Google/DuckDuckGo.
- [ ] Empty Tavily results → chain advances via existing empty-result detection.
- [ ] 401/403 invalid key raises `ConfigError` → propagates out, run stops (does
      **not** fall through).
- [ ] Credentials read via `config.py`; nothing secret in code or commits.
- [ ] Offline tests (faked `httpx`, DI'd client) cover the above; no live calls
      in the default `pytest` run.
- [ ] `uv run ruff check .`, `uv run ruff format`, `uv run mypy src`,
      `uv run pytest` all pass.
- [ ] `example.env` present and accurate.
- [ ] `python-dotenv` declared; `load_dotenv()` runs at both entrypoints before
