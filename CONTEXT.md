...
Tenant data, checkpoints, guardrails, the knowledge library and the questionnaire move into Postgres behind a `DocumentStore` port; the eight rules, the pipeline definition and the specialist prompts stay as code-shipped markdown (see [ADR-0014](docs/adr/0014-postgres-system-of-record-and-split-governance.md)). The paths above describe the current filesystem layout, which the local-development adapter continues to serve.
