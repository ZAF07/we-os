...
- Refined by [ADR-0024](0024-platform-owned-tenant-ids-and-explicit-run-abandonment.md), which settles four things this ADR left implicit: the platform mints its own tenant id and keeps the IdP's in an `external_auth_id` column; checkpoint threads are keyed by tenant as well as slug; abandoning a run explicitly clears its checkpoint threads and a heartbeat distinguishes a crashed worker's runs from a live peer's; and the schema is provisioned by an operator command rather than by the service, which connects as a non-superuser role so row-level security constrains it.
