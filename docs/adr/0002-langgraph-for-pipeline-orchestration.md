...
Status: accepted — supersedes the hand-rolled loop (`8c05f37`) and the Google ADK version (`3998d9d`, `467c321`, tagged `archive/adk-framework-base`).
...
Two prior designs were tried and dropped: the original had a bespoke `loop/` + `orchestrator.py` + `providers/` (`8c05f37`); the second built on Google ADK (`3998d9d`, `467c321`). The LangGraph rewrite (`d5d5ed6` "first refactor to langgraph", then `d5b1dfc`, `863f255`) deleted both and moved the code under `src/marketing_os/graph/`. The ADK work is **retired**. It was the first proof of concept and its branch has been deleted; the two commits survive only under the tag `archive/adk-framework-base`, as historical reference rather than a maintained fallback.
