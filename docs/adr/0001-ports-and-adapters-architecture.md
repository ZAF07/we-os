...
The one first-class port is the QA judge: `Reviewer` (a `runtime_checkable` `Protocol`) in `agent-harness/src/marketing_os/ports.py:17`. Adapters live under `agent-harness/src/marketing_os/adapters/` (`models.py`, `review.py` implementing `Reviewer`, `observability/`, `tools/`). Dependency injection is via `build_campaign_graph(settings, *, model=, reviewer=, web_backend=, checkpointer=)` in `graph/graph.py`; tests inject `FakeReviewer` and a scripted `ProgrammableChatModel` (see `tests/conftest.py`).
